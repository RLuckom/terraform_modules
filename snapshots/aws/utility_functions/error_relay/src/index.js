...
function constructReadableError(evt) {
  ...
  try {
    ...
    ddMetadata.time = _.get(ddMetadata, 'apiError.time') || _.get(evt, 'requestPayload.eventTime') || new Date().toISOString()
    ddMetadata.functionArn = _.get(evt, 'requestContext.functionArn')
  } catch(e) {
  }
  let ddMetaString = ''
  if (ddMetadata) {
