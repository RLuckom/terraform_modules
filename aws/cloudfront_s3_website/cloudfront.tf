...
resource "aws_cloudfront_distribution" "website_distribution" {
  ...
  dynamic "ordered_cache_behavior" {
    for_each = [ for origin in var.lambda_origins : origin if (origin.authorizer == "CLOUDFRONT_DISTRIBUTION" || length(values(var.lambda_authorizers)) < 1) && origin.authorizer != "NONE" ]
    content {
      path_pattern = ordered_cache_behavior.value.path
      target_origin_id = ordered_cache_behavior.value.gateway_name_stem
      allowed_methods = ordered_cache_behavior.value.allowed_methods
  ...
  dynamic "ordered_cache_behavior" {
    for_each = [ for origin in var.lambda_origins : origin if origin.authorizer == "NONE" ]
    content {
      path_pattern = ordered_cache_behavior.value.path
      target_origin_id = ordered_cache_behavior.value.gateway_name_stem
      allowed_methods = ordered_cache_behavior.value.allowed_methods
  ...
  dynamic "ordered_cache_behavior" {
    for_each = var.no_access_control_s3_path_patterns
    content {
      path_pattern = ordered_cache_behavior.value.path
      target_origin_id = local.s3_origin_id
      allowed_methods  = ["GET", "HEAD", "OPTIONS"]
